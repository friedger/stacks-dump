...
var jsonStream = fs.createWriteStream("report.json", { flags: "w" });
var csvStream = fs.createWriteStream("report.csv", { flags: "w" });
var txtStream = fs.createWriteStream("report.txt", { flags: "w" });
...
function process_staging_blocks() {
  ...
  for (let row of result) {
    ...
    if (burn_blocks_by_consensus_hash[row.consensus_hash]) {
      burn_blocks_by_consensus_hash[row.consensus_hash].staging_blocks.push(
        row
      );
    } else {
      console.log("missing consensus hash", row.consensus_hash);
  ...
  for (let row of result) {
    ...
    if (op.LeaderBlockCommit && op.LeaderBlockCommit.apparent_sender) {
      ...
      if (
        ...
        op.LeaderKeyRegister.block_height < end_block
      ) {
        console.log(
