...
const stmt_all_block_headers = headers_db.prepare(
  ...
  for (let row of result) {
    ...
    if (op.LeaderBlockCommit) {
      ...
      op.LeaderBlockCommit.public_key = secp256k1
        .publicKeyConvert(
          Buffer.from(op.LeaderBlockCommit.input.public_keys[0].key, "hex"),
          op.LeaderBlockCommit.input.public_keys[0].compressed
        )
        .toString("hex");
...
(async () => {
  ...
  for (let block of burn_blocks_by_height) {
    ...
    if (
      ...
      block.payments[0].stacks_block_height > stacks_block_height_max
    ) {
      stacks_block_height_max = block.payments[0].stacks_block_height;
      at_tip = ">";
    }
    const current_winner_address = block.block_commits.find(
    ...
    const stacks_block_id = block.block_headers.length
      ...
      : "-";
    const txids =
      block.block_headers.length && use_txs
    ...
    console.log(
      ...
      block.block_headers.length
        ? `${block.block_headers[0].block_height}`
        : "-",
      block.branch_info
        ? `${block.branch_info.name}${block.on_winning_fork ? "$" : " "}`
        : " ",
      ...
      block.block_headers.length
        ? `b:${block.block_headers[0].burn_header_hash.substring(0, 10)}`
        : "-",

      block.block_headers.length
        ? `${
            block.block_headers[0].parent_block === parent_hash
              ? (parent_winner_address
                  ? parent_winner_address.leader_key_address
                  : null) ===
                (current_winner_address
                  ? current_winner_address.leader_key_address
                  : null)
                ? "@+"
                : "@@"
              : "  "
  ...
  console.log(
    "STX address/BTC address - actual wins/total wins/total mined %won %actual wins - paid satoshis Th[theoritical win%] (avg paid)"
  );
  ...
  console.log(`Miners seen at least once: ${Object.keys(miners).length}`);
})();
